...
Assertions take conditional expressions an *assert* that they should be `true`, otherwise it'll stop the code.
...
In reality you don't often need it (some may say bad practice), but in some situations a do-while statement may look more elegant/reflect the reality of the situation better.
...
Switch statements may be helpful for situations where you have multiple different things you want to do that is dependent on some expression.
...
Write a function that goes through `String[][] tweets` where the first dimension is particular user, and `String[i][] = new String[]{"jackiec3", "Hello Twitter!"}`, i.e. the second dimension contains two strings, the username and tweet.
