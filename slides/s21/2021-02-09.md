...
Can void return functions return (i.e. will the code below run)?
```java
...
- Strings are the Java data type for working with text.

- This is our first example of a Java data object. Java objects combine together **state** and **behavior**.
...
System.out.println(full.length()); //how this this different from the method used to find the length of an array?
```
...
Create a function called combineStrings that will output a new String in the form short+long+short where short is the shorter of the two strings and long is the longer of the two strings. The two strings will not be off the same length. 
...
Can void return functions return? (i.e. will the code below run without errors?) Yes, void functions can still return so this code will run fine. The return will just exit the function, but it will not actually return anything. 
```java
void isNumTwo(int num) {
  if (num == 2) {
    System.out.println("Yes, this number is two.");
