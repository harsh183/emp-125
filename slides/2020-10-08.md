...
---
# Dot Notation
* used to access object fields and methods
```Java
public class Bird {
  public int wings = 0;
...
* create one function ```Upgrade()``` that will change an ECE major to a CS major
* create anoter function ```graduate()``` that will change their grad year to 2020.
